...
function buzznews_customize_general_settings( $wp_customize ) {
   ...
   //Enable  Slider
    ...
    $wp_customize->add_control( 
        'buzznews_post_timedate', 
        array(
            'label' => esc_html__( 'Date Format', 'buzznews' ),
            'section' => 'buzznews_timeanddate',
            'type' => 'select',
            'choices' => array(
                            'human-readable'      => esc_html__( 'Format Two', 'buzznews' ),
                            'default'             => esc_html__( 'Format One ( Default )', 'buzznews' ),
            ),
            'priority'          => 3,
        )
    );
    ...
    $wp_customize->add_control(
		new BuzzNews_Toggle_Control( 
			...
			array(
				...
				'label'		  => esc_html__( 'Enable Sticky Header', 'buzznews' ),
                'description' => esc_html__( 'Enable/Disable Sticky Header', 'buzznews' ),
                'priority'    => 1
			)
		)
    );
